...
export default function EQSurvey() {
  ...
  const processCalculations = () => {
    ...
    questions.forEach((q) => {
      ...
      const domain = q.domainName as keyof typeof domainTotals;
    ...
    // Sort domains to isolate highest and lowest metrics
    ...
    setStep(3); // Advance to the Playbook step once math completes
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) setStep(4); // Advance to final results dashboard card
    } catch (err) {
  ...
  if (step === 4 && calculatedMetrics) {
    return (
      <div style={{ padding: "10px", fontFamily: "sans-serif", color: "#111" }}>
        <div style={{ textAlign: "center", marginBottom: "30px", background: "#F0FDF4", border: "1px solid #BBF7D0", padding: "20px", borderRadius: "12px" }}>
          <h2 style={{ fontSize: "22px", color: "#166534", margin: "0 0 5px 0", fontWeight: "bold" }}>Playbook Generated Successfully! 🎉</h2>
          <p style={{ color: "#166534", fontSize: "14px", margin: 0 }}>Thank you, {fullName}. Your profile data and real-world experiments have been securely recorded.</p>
        </div>

        <h3 style={{ fontSize: "18px", fontWeight: "bold", borderBottom: "2px solid #E5E7EB", paddingBottom: "8px", marginBottom: "20px" }}>Your Emotional Intelligence Core Profile</h3>
        
        {/* Metric Bar Matrix */}
        <div style={{ spaceY: "15px", marginBottom: "30px" }}>
          {Object.entries(calculatedMetrics.breakdown).map(([key, value]) => (
            <div key={key} style={{ marginBottom: "15px" }}>
              <div style={{ display: "flex", justifyBetween: "space-between", justifyContent: "space-between", fontSize: "13px", fontWeight: "600", marginBottom: "4px" }}>
        ...
        <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", padding: "15px", borderRadius: "8px", marginBottom: "15px" }}>
          <h4 style={{ margin: "0 0 5px 0", fontSize: "15px", color: "#1E40AF", fontWeight: "bold" }}>🌟 Core Leadership Strength: {calculatedMetrics.strength.name}</h4>
          <p style={{ margin: 0, fontSize: "13px", color: "#1E3A8A", lineHeight: "1.5" }}>{calculatedMetrics.strength.desc}</p>
        </div>

        {/* Growth Horizon Card */}
        <div style={{ background: "#FFFBEB", border: "1px solid #FEF3C7", padding: "15px", borderRadius: "8px", marginBottom: "25px" }}>
          <h4 style={{ margin: "0 0 5px 0", fontSize: "15px", color: "#92400E", fontWeight: "bold" }}>🎯 Primary Growth Horizon: {calculatedMetrics.growth.name}</h4>
          <p style={{ margin: 0, fontSize: "13px", color: "#78350F", lineHeight: "1.5" }}>{calculatedMetrics.growth.desc}</p>
        </div>

        <div style={{ background: "#F9FAFB", border: "1px solid #E5E7EB", padding: "15px", borderRadius: "8px", textAlign: "center" }}>
          <p style={{ margin: 0, fontSize: "13px", color: "#4B5563" }}>An activation check-in loop will initialize via <strong>{email}</strong> in 14 days to audit your experiment adjustments.</p>
        </div>
      </div>
    );
  }
