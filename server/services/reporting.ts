import { db } from "../db";
import { surveyCycles, surveyResponses, users } from "@shared/schema";
import { eq } from "drizzle-orm";

// Define the 6 explicit dimensions matching the SyncShift Framework [cite: 3, 6, 20, 31, 42, 53, 64]
...
export async function generateSyncShiftReportData(cycleId: number): Promise<AggregatedReportData> {
  ...
  // 2. Fetch All Submitted Responses using the exact schema table name
  ...
  // Initialize data accumulator matrices [cite: 6, 20, 31, 42, 53, 64]
  ...
  // 4. Run the Modulo-6 Interleaved Matrix Processor over your exact column data layout
  allResponses.forEach((response) => {
    // Read from response.responses payload matching your database records
    ...
    answersList.forEach((item) => {
      ...
      const score = parseInt(item.value); // Maps to string values ('6', '5', '7') generated by seeder
      ...
      if (dimensionKey && dimensionAccumulator[dimensionKey]) {
        if (isSelf) {
          dimensionAccumulator[dimensionKey].selfSum += score;
          dimensionAccumulator[dimensionKey].selfCount += 1;
        } else {
          dimensionAccumulator[dimensionKey].extSum += score;
